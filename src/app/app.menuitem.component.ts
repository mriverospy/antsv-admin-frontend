...
export class AppMenuitemComponent implements OnInit, OnDestroy {
            ...
            .subscribe(params => {
                ...
                if (this.app.isHorizontal() || this.app.isSlim()) {
                    ...
                    if (this.item.routerLink) {
                        ...
                        // Keep parent menus active if any of their children routes are active
                        this.updateActiveStateFromChildRoutes();
                    } else {
                        this.active = false;
                    }
                }
    ...
    updateActiveStateFromChildRoutes() {
        // Check if any child route is currently active
        if (this.item.items) {
            this.active = this.item.items.some((child: any) => {
                if (child.routerLink) {
                    return this.router.isActive(child.routerLink[0], true);
                } else if (child.items) {
                    // Recursively check nested children
                    return this.checkNestedChildRoutes(child);
                }
                return false;
            });
        }
    ...
    private checkNestedChildRoutes(item: any): boolean {
        if (item.items) {
